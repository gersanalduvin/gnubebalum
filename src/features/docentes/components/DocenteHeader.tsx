...
import { Avatar, Box, Button, Chip, CircularProgress, Grid, IconButton, Paper, Typography } from '@mui/material'

import type { DocenteFormData } from '../types'
...
export default function DocenteHeader({
  ...
  return (
    <Paper
      elevation={2}
      sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)', borderRadius: 2 }}
    >
              ...
              {isUploading && (
                ...
                >
                  <CircularProgress
                    size={40}
                    thickness={4}
                    sx={{ color: 'primary.main', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))' }}
                  />
                </Box>
              )}
            </Box>
                ...
                {photoUrl && onPhotoDelete && !isUploading && (
                  <IconButton
                    ...
                    sx={{
                      bgcolor: 'error.light',
                      color: 'white',
                      '&:hover': { bgcolor: isDeleting ? 'error.light' : 'error.main' }
                    }}
                  >
