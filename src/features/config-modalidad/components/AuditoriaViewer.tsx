...
interface AuditoriaViewerProps {
  open: boolean
  modalidad: ConfigModalidad | null
  onClose: () => void
}

const AuditoriaViewer: React.FC<AuditoriaViewerProps> = ({ open, modalidad, onClose }) => {
  // Parsear los cambios si vienen como string
  const parsedCambios: CambioAuditoria[] = React.useMemo(() => {
    if (!modalidad?.cambios) return []
    
    if (typeof modalidad.cambios === 'string') {
      try {
        return JSON.parse(modalidad.cambios)
      } catch {
        return []
      }
    }
    return Array.isArray(modalidad.cambios) ? modalidad.cambios : []
  }, [modalidad?.cambios])
  ...
  return (
    ...
    >
      <DialogTitle>
        Historial de Cambios - {modalidad?.nombre}
      </DialogTitle>
      <DialogContent>
        {renderContent()}
      </DialogContent>
