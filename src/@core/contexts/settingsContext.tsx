...
export const SettingsProvider = (props: Props) => {
  ...
  // State - Initialize with server data to avoid hydration mismatch
  const [_settingsState, _updateSettingsState] = useState<Settings>(
    props.settingsCookie && Object.keys(props.settingsCookie).length > 0 ? props.settingsCookie : updatedInitialSettings
  )
  ...
  // Sync client-side cookie with state after hydration
  useEffect(() => {
    if (settingsCookie && Object.keys(settingsCookie).length > 0) {
      const cookieString = JSON.stringify(settingsCookie)
      const stateString = JSON.stringify(_settingsState)
      
      if (cookieString !== stateString) {
        _updateSettingsState(settingsCookie)
      }
    }
  }, [settingsCookie, _settingsState])
