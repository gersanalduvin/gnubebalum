...
export const confirmUrlInChildren = (children: ChildrenType['children'], url: string): boolean => {
  ...
  if (isValidElement(children)) {
    const { component, href, exactMatch = false, activeUrl, children: subChildren } = children.props

    if (subChildren) {
       if (confirmUrlInChildren(subChildren, url)) return true
    }

    if (component && component.props.href) {
       const target = component.props.href
       if (exactMatch) return target === url
       const checkUrl = activeUrl || target
       return url === checkUrl || url.startsWith(checkUrl + '/')
    }

    if (href) {
       if (exactMatch) return href === url
       const checkUrl = activeUrl || href
       return url === checkUrl || url.startsWith(checkUrl + '/')
    }
  }
